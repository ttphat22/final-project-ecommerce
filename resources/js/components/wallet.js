...
var Wallet = {
	...
	validateForm()
	{
		...
		$(".js-save-wallet").click(function (event) {
			...
			if (check <= 0)
			{
				$("#money_online").next().html('Please enter your deposit amount')
				return  false;
			}else if (check < 6){
				$("#money_online").next().html('The minimum value is 10,000 VND')
				return  false;
			}
			...
			if ($("#popup-wallet .type-atm").hasClass('active')) {
				...
				if (name == undefined) {
					toast.warning("Please choose the payment bank");
					return  false;
				}
			}
